...
public class AssignmentsCreator {
    ...
    public void makeAssignments() {
        for (int i = 0; i < CourseTable.getCourses().size(); i++) {
            for (int j = 1; j <= 15; j++) {
                if (j <= 12) {
                    assignments.add(new Assignments(1, i, "Minor", j, (int) (Math.random() * 26) + 75)); // Replace student id with one from CourseTable
                } else {
                    assignments.add(new Assignments(1, i, "Major", j, (int) (Math.random() * 26) + 75)); // Replace student id with one from CourseTable
                }
            }
        }
    }
